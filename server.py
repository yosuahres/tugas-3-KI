...
private_key = {'d': 103, 'n': 143}
public_key = {'e': 7, 'n': 143}
...
def key_des_encrypt(message, identity):
    signature = []
    for letter in message:
       signature.append(signature_encrypt(ord(letter)))
    
    encrypted = []
    for sign in signature:
        encrypted.append(rsa_encrypt(sign, identity))
...
def get_public_key(identity):
    ...
    if (public_key != "Identity not found"):
...
def handshake_responder(conn):
    n2 = "test2"

    data = conn.recv(1024).decode()
    data_rcv = json.loads(data)

    iniator_identity = rsa_string_decoder(data_rcv[0])
    data_rcv_n1 = rsa_string_decoder(data_rcv[1])

    if iniator_identity == "Minta Key":
        key_permission = iniator_identity
        iniator_identity = data_rcv_n1

        key_encrypted = key_des_encrypt(des_key, iniator_identity)
        conn.send(json.dumps(key_encrypted).encode())

        return

    print("Data ID dan n1 telah diterima")
    print(f"ID diterima: {iniator_identity}")
    print(f"n1 diterima: {data_rcv_n1} \n")

    if iniator_identity not in client_public_key:
        get_public_key(iniator_identity)

    n1_encrypted = rsa_string_encoder(data_rcv_n1, iniator_identity)
    n2_encrypted = rsa_string_encoder(n2, iniator_identity)

    print(f"n1: {data_rcv_n1}")
    print(f"n2: {n2}")

    data_to_send = [n1_encrypted, n2_encrypted]

    conn.send(json.dumps(data_to_send).encode())

    print("Data n1 dan n2 telah dikirim \n")

    data = conn.recv(1024).decode()
    data_rcv = json.loads(data)

    data_rcv_n2 = rsa_string_decoder(data_rcv[0])

    print("Data n2 telah diterima")
    ...
    if n2 == data_rcv_n2 :
        ...
        key_encrypted = key_des_encrypt(des_key, iniator_identity)
        conn.send(json.dumps(key_encrypted).encode())
        print(f"Key Des dikirim: {des_key}\n")
